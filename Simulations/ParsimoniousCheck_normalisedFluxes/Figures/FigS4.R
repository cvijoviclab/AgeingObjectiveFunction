...
# reduce data to oxidative stress, and interesting fluxes
oxStress <- subset(diff, pathway == "Oxidative stress" &
                       grepl("transport", reaction) == FALSE &
                       grepl("pseudo", reaction) == FALSE &
                       grepl("exchange", reaction) == FALSE)

# remove distinction between compartments
for (i in 1:nrow(oxStress)) {
  if (grepl("(", oxStress$reaction[i], fixed = TRUE)) {
    newReactionNames <- sub(' \\(.*', '', oxStress$reaction[i])
    oxStress$reaction[i] <- newReactionNames
  }
}

# plot flux changes in oxidative stress pathway in phase I
fig <- ggplot(subset(oxStress, phase == "phase I"),
              aes(x = reaction, y = relDiff, fill = objective, color = objective)) +
  my_theme + 
  ...
  xlab("reaction") +
  ylab("relative changes of fluxes") +
...
ggsave(file = "S4/oxStress_I.svg", plot = fig)

# plot flux changes in oxidative stress pathway in phase II
fig <- ggplot(subset(oxStress, phase == "phase II"),
              aes(x = reaction, y = relDiff, fill = objective, color = objective)) +
  my_theme + 
  geom_hline(yintercept = 0, color = "black") +
  geom_boxplot(fill = "black", color = "black", alpha = 0.2,
  ...
  scale_fill_manual(values = my_colours) +
  theme(legend.position =  "none")
ggsave(file = "S4/oxStress_II.svg", plot = fig)

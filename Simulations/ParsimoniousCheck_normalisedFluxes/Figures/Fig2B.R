...
# plot relative flux difference for phase I
fig <- ggplot(data = subset(diff, phase == "phase I"),
              aes(x = pathway, y = relDiff, fill = objective, color = objective)) +
  my_theme + 
  ...
  scale_y_continuous(breaks = c(-1, 1, 3, 5)) +
  scale_color_manual(values = my_colours) +
  scale_fill_manual(values = my_colours) +
  theme(legend.position =  "none")
ggsave(file = "2B/relDiff_I.svg", plot = fig)

# plot relative flux difference for phase II
fig <- ggplot(data = subset(diff, phase == "phase II"),
  ...
  scale_fill_manual(values = my_colours) +
  theme(legend.position =  "none")
ggsave(file = "2B/relDiff_II.svg", plot = fig)
